...
class VoiceCommandSegmenter:
    ...
    def process(self, chunk_seconds: float, speech_probability: float | None) -> bool:
        ...
        Returns False when command is done (finished or timed out).
        """

        if self.timed_out:
            self.timed_out = False

        # Global timeout countdown (no clamping)
        self._timeout_seconds_left -= chunk_seconds

        if self._timeout_seconds_left <= 0:
            return self._finish_timeout()

        p = speech_probability or 0.0

        if not self.in_command:
            # Before command
            self._handle_before_command(chunk_seconds, p)
            return True

        # In command
        return self._handle_in_command(chunk_seconds, p)

    def _handle_before_command(
        self, chunk_seconds: float, speech_probability: float
    ) -> None:
        """Handle state when we are not yet in a command."""
    ...
    def _handle_in_command(
        ...
        # Speech in command
        # Reset silence counter if enough speech is detected
        self._reset_seconds_left -= chunk_seconds
        self._command_seconds_left -= chunk_seconds
